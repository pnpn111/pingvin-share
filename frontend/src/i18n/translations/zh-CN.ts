export default {
  ...
  "share.fileCount": "{count} 个文件 · {size}（压缩文件可能因服务器重新压缩而比标示的小）",
  ...
  "admin.config.cache.ttl": "TTL (生存时间)",
  "admin.config.cache.ttl.description": "缓存信息的保留时间（单位：秒）。",
  "admin.config.cache.max-items": "最大项目数",
  "admin.config.cache.max-items.description": "缓存中允许存储的最大项目数量。",
  "admin.config.cache.redis-enabled": "启用 Redis",
  "admin.config.cache.redis-enabled.description": "通常情况下，Pingvin Share 会将信息缓存在内存中。如果您运行多个 Pingvin Share 实例，则需要启用 Redis 缓存以在实例之间共享缓存数据。",
  "admin.config.cache.redis-url": "Redis URL",
  "admin.config.cache.redis-url.description": "用于连接到 Redis 缓存实例的 URL。",
  ...
  "admin.config.ldap.admin-groups.description": "管理访问权限所需的群组。",
  "admin.config.ldap.field-name-member-of": "用户群组属性名称",
  "admin.config.ldap.field-name-member-of.description": "用户所属群组的 LDAP 属性名称。此属性用于检查管理员群组。",
  "admin.config.ldap.field-name-email": "用户邮箱属性名称",
  "admin.config.ldap.field-name-email.description": "表示用户邮箱的 LDAP 属性名称。",
  ...
  "admin.config.s3.endpoint.description": "S3 存储桶的 URL。",
  "admin.config.s3.region": "区域",
  "admin.config.s3.region.description": "S3 存储桶所在的区域。",
  ...
  "admin.config.s3.use-checksum.description": "对于不支持校验和的后端（例如 B2），请禁用此选项。",
